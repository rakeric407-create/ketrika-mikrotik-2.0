# -*- coding: utf-8 -*-
"""
KETRIKA MIKROTIK 301 - Generateur .rsc CORRIGE
- Wi-Fi : set DIRECT par numero d index [0] et [1] (pas par nom)
- WireGuard : genere TOUJOURS avec cle locale (pas besoin API externe)
- NAT : cree EN PREMIER avant tout mangle
- Connexion : garantie meme si WireGuard echoue (fallback WAN)
"""
...
def _gen_wg_privkey():
    """Genere une cle privee WireGuard valide (curve25519, 32 bytes base64)"""
...
# Cle publique du serveur WARP Cloudflare (fixe, publique)
...
def generate_rsc(order, model_info):
    """Genere le script .rsc RouterOS v7"""
    ...
    a(f"# Import  : /import file-name={fn}")
    a("")
    a(":log warning \"KETRIKA 301 : Demarrage configuration...\"")
    a(":delay 1s")
    a("")
    ...
    # === NAT MASQUERADE EN PREMIER (garantit la connexion) ===
    a("# --- NAT Masquerade WAN (PRIORITAIRE) ---")
    ...
    # === WI-FI CORRIGE : SET PAR INDEX NUMERIQUE ===
    if has24 or has5:
        a("# ============================================")
        ...
        if wtype == "ax":
            ...
            a("")
            a("# Appliquer par index (0 = premiere interface, 1 = deuxieme)")
            a(":local wlist [/interface/wifi/find]")
            a(":if ([:len $wlist] > 0) do={")
            a("  /interface/wifi/set ($wlist->0) configuration=kcfg2 disabled=no")
            a("}")
            if has5:
                a(":if ([:len $wlist] > 1) do={")
                a("  /interface/wifi/set ($wlist->1) configuration=kcfg5 disabled=no")
                a("}")
            a("")
            a("# Activer toutes les interfaces wifi")
            ...
            a("")
            a("# Appliquer par index (0 = wlan1, 1 = wlan2)")
            a(":local wlist [/interface/wireless/find]")
            a(":if ([:len $wlist] > 0) do={")
            a(f"  /interface/wireless/set ($wlist->0) mode=ap-bridge ssid=\"{s2}\" security-profile=ksec band=2ghz-b/g/n channel-width=20/40mhz-XX frequency=auto disabled=no")
            a("}")
            if has5:
                a(":if ([:len $wlist] > 1) do={")
                a(f"  /interface/wireless/set ($wlist->1) mode=ap-bridge ssid=\"{s5}\" security-profile=ksec band=5ghz-a/n/ac channel-width=20/40/80mhz-XXXX frequency=auto disabled=no")
                a("}")
            a("")
            a("# Activer toutes les interfaces wireless")
        ...
        a("")
        a(f":log warning \"KETRIKA : Wi-Fi OK - SSID={s2} MDP={wp}\"")
        a("")

    # === WIREGUARD WARP (plan 2+) - CLE GENEREE LOCALEMENT ===
    if plan in ("performance", "business"):
        ...
        a("# ============================================")
        a("")
        a("# Nettoyage complet ancien WireGuard")
    ...
    if plan == "business" and order.hotspot_tickets_count and order.hotspot_tickets_count > 0:
        a("# --- Hotspot ---")
        a(f":if ([:len [/ip/hotspot/profile/find where name=\"hsprof\"]] = 0) do={{ /ip/hotspot/profile/add name=hsprof hotspot-address={gw} dns-name=login.ketrika.mg html-directory=hotspot login-by=http-chap,http-pap }}")
        for pn, to, rl in [("h1h","1h","5M/10M"),("h1d","1d","5M/10M"),("h1w","1w","5M/10M"),("h1m","4w2d","10M/20M")]:
            a(f":if ([:len [/ip/hotspot/user/profile/find where name=\"{pn}\"]] = 0) do={{ /ip/hotspot/user/profile/add name={pn} session-timeout={to} shared-users=1 rate-limit={rl} }}")
        a(":if ([:len [/ip/hotspot/find where name=\"hs1\"]] = 0) do={ /ip/hotspot/add name=hs1 interface=bridge address-pool=pool1 profile=hsprof disabled=no }")
        a("")
        for _ in range(min(order.hotspot_tickets_count, 200)):
            ...
            a(f":if ([:len [/ip/hotspot/user/find where name=\"{u}\"]] = 0) do={{ /ip/hotspot/user/add name={u} password={p} profile=h1d server=hs1 }}")
        a("")
    ...
    for ch, act, par, tag in [
        ...
        a(f":if ([:len [/ip/firewall/filter/find where comment=\"K-{tag}\"]] = 0) do={{ /ip/firewall/filter/add chain={ch} action={act} {par} comment=\"K-{tag}\" }}")
    a("")
    ...
    if has24:
        a(f":put \"  Wi-Fi   : {s2}\"")
        a(f":put \"  Mot de passe : {wp}\"")
    a(f":put \"  IP      : {gw}\"")
    if plan in ("performance", "business"):
        a(":put \"  VPN WARP : actif\"")
    a(":put \"  Support : wa.me/261382817100\"")
    a(":put \"================================================\"")
    a(":log warning \"KETRIKA 301 : Configuration terminee avec succes\"")
...
def generate_tutorial_txt(order, filename):
    ...
    L = [
        ...
        f"  ETAPE 3 : Glisser {filename} dans Files",
    ...
    if order.wifi_password:
        L.append(f"  Mot passe : {order.wifi_password}")
    L.extend(["", "  Support : wa.me/261382817100", "  KETRIKA MIKROTIK Madagascar", "================================================"])
    return "\n".join(L)
